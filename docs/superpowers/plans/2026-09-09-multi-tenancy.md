...
| 报错的名字 | 改法 |
|---|---|
| `messages` / `studyRoomUsers` / `treeHolePosts` / `posts` 出现在 `io.on("connection", ...)` 闭包内 | 在该闭包最顶部（`console.log("User connected"...)` 之后）加一行 `const { messages, studyRoomUsers, treeHolePosts, posts } = state();`，闭包内其余引用不动。**前提是连接时已有用户上下文**——Task 6 会在 `io.use` 里建立；本任务先在这行之前加 `const st = runWithUser(LOCAL_USER_ID, () => state());` 的临时写法**不允许**，改为直接 `state()`：单人模式下 startServer 已跑在 local 上下文里（Task 2 Step 4），连接闭包继承它。 |
...
```ts
const AUTH_EXEMPT = ["/api/auth/", "/api/health", "/api/extension/pair"];
