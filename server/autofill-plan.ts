 ...
 * 高度特异，而大多数中文姓名就是 2-3 个字。真机上「张小明」因此被判 unsourced，
 * 最基本的字段填不上。
 */
 ...
 * `- **姓名**: 张小明`，模型引「姓名: 张小明」，只去空白的话原文是
 * 「姓名**:张小明」，对不上，最基本的字段被自己的闸门判成 unsourced。
 *
...
export function buildAutofillPrompt(input: {
  ...
  return [
    ...
    "5. 工作描述、项目描述这类成段的框，**直接把档案里对应的原文抄过去**——不要总结、不要改写、不要压缩成一句话。档案里那些内容本来就是为简历写的。",
    "6. 其余允许的加工只有：提取已展示的技能、缩写或翻译。加工不得改变事实。",
...
export function validateAutofillPlan(
  ...
  for (const raw of items) {
     ...
     * 引用必须包含要填的值——「指给我看这个值出自哪里」。
     *
     * 曾经为了让「工作描述」能填上而对长文本放开这一条，那是解错了题：以为这类
     * 字段天然要改写，所以压出来的句子不可能在原文里逐字出现。其实**它本来就该
     * 直接照抄**——档案里那几条 bullet 就是为简历写的。让它抄，这条约束自然成立，
     * 闸门一点都不用松。
     *
     * 而放松它恰恰是在最该防的地方开口子：大段自由文本正是编造成本最低、也最难
     * 被人一眼看穿的地方。
     *
     * 有 options 的字段例外：值来自表单给的选项（已在上面校验过命中），引用的职责
     * 只是证明选这个选项的依据在档案里，比如据「双硕士项目」选「研究生」。
     */
