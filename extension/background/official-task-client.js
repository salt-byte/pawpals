import { SERVER_BASE, resolveBase } from './session-client.js';

export function createOfficialTaskClient({ fetchImpl, base = SERVER_BASE } = {}) {
  const doFetch = fetchImpl ?? globalThis.fetch;
  return {
    async next() {
      const response = await doFetch(`${await resolveBase(base)}/api/internal/official-application-task`);
      if (!response.ok) throw new Error(`任务服务返回 ${response.status}`);
      return (await response.json()).task || null;
    },
    async complete(id, result) {
      const response = await doFetch(`${await resolveBase(base)}/api/internal/official-application-task-done`, {
    ...
    async reportContext(payload) {
      try {
        await doFetch(`${await resolveBase(base)}/api/internal/official-application-context`, {
